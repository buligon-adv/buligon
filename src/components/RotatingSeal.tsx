...
export default function RotatingSeal() {
  return (
    <div className="relative w-32 h-32 md:w-48 md:h-48 flex items-center justify-center p-2 group">
      {/* Efeito de Vidro (Glassmorphism) no Fundo - Recuado para não cortar o texto */}
      <div className="absolute inset-8 bg-white/[0.03] backdrop-blur-[6px] rounded-full border border-white/10 shadow-[inner_0_1px_1px_rgba(255,255,255,0.1)] -z-10" />
      
      {/* Sombra de destaque externa */}
      <div className="absolute inset-10 bg-black/20 rounded-full blur-2xl -z-20" />
      ...
      {/* Decorativo: Círculos concêntricos - Ajustados para não interferir no texto */}
      <div className="absolute inset-8 border border-bronze/20 rounded-full" />
      <div className="absolute inset-10 border border-bronze/10 rounded-full" />
